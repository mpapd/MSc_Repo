...
public:
	...
	cacheList * GetWndCache();   //Nel
	cacheInstList * GetWndInstCache();   //inst cache
...
protected:
	...
	afx_msg void OnAboutCachesimulator();
	afx_msg void OnCacheConfiguration();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
private:
	...
	cacheList	m_wndCache;   //Nel
	cachestatic m_wndTextCache;
	cacheInstList m_wndInstCache;  //inst cache
	cachestatic m_wndTInstCache;
	...
	void Initialize();
	void DisplayCache();   //Nel
	void DestroyCacheWindows(int old_data_sim,int old_inst_sim,int old_show,int old_showI);
	void cacheConfigChange(int old_cache_sim);
};
