...
% 6 είσοδοι: y(k-1), y(k-2), u(k)
%--------------------------------------------------------------------------
close all;      clear all;      clc;        format long;
%__________________________________________________________________________
%__________________________________________________________________________
% ΠΑΡΑΜΕΤΡΟΙ ΜΟΝΤΕΛΟΥ - ΕΚΠΑΙΔΕΥΣΗΣ
fuzzySets = 3;                      % ασαφή σύνολα ανά είσοδο
epoch_number = 120;                 % αριθμός επαναλήψεων εκπαίδευσης
...
for t = 1:samples 
    Data(t,:) = [BJ_data(t,2) BJ_data(t,3), BJ_data(t,4), BJ_data(t,5)]; 
end
trnData = Data(1:samples,:);
save('trnData.dat','trnData','-ASCII');

fismat = genfis1(trnData,[fuzzySets fuzzySets fuzzySets],char(mfType,mfType,mfType),char(outType));
%-------------------------------------------------------------------------- 
...
anfis_output = evalfis(trnData(:,1:3), fismat2);
index = 1:samples;
