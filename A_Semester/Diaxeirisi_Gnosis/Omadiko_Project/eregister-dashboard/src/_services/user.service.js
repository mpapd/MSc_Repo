...
function getAll() {
    ...
    return fetch(`https://eregister-backend.herokuapp.com/orchestrator/applications`, requestOptions).then(handleResponse);
}

function getSchools(town, district) {
    const requestOptions = { method: 'GET', headers: authHeader() };
    return fetch(`https://eregister-backend.herokuapp.com/orchestrator/schools/` + town +`/`+district+`?category=1`, requestOptions).then(handleResponse);
}

function getMusicSchools(town) {
    const requestOptions = { method: 'GET', headers: authHeader() };
    return fetch(`https://eregister-backend.herokuapp.com/orchestrator/schools/` + town +`?type=2`, requestOptions).then(handleResponse);
}

function getAthleticSchools(town) {
    const requestOptions = { method: 'GET', headers: authHeader() };
    return fetch(`https://eregister-backend.herokuapp.com/orchestrator/schools/` + town +`?type=3`, requestOptions).then(handleResponse);
}
...
function saveApplication(firstname, lastname, fathername, mothername, birthdate, town, townEng, district, districtEng, street,foreignLanguage1,foreignLanguage2,optionalClass1,optionalClass2,school_id,comments ) {
    ...
    return fetch(`https://eregister-backend.herokuapp.com/orchestrator/application`, requestOptions
