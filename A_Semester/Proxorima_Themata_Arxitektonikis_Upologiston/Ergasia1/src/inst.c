...
inst_decode (int32 val)
{
  ...
  switch (map_int_to_name_val_val (name_tbl,
    ...
    case FP_R2ds_TYPE_INST:
      return (mk_r_inst (val, i_opcode, BIN_FS(val), 0 , BIN_FD(val), BIN_SA(val)/*0 Modificat Nel*/));

    case FP_R2ts_TYPE_INST:
      return (mk_r_inst (val, i_opcode, 0 , BIN_RT(val), BIN_FS(val) , BIN_SA(val)/*0 Modificat Nel*/));
    ...
    case FP_R3_TYPE_INST:
      return (mk_r_inst (val, i_opcode, BIN_FS(val) , BIN_FT(val) , BIN_FS(val)/*BIN_FD(val) Modificat Nel*/, BIN_SA(val)/*0 Modificat Nel*/));

    case FP_MOVC_TYPE_INST:
      return (mk_r_inst (val, i_opcode, BIN_FS(val), BIN_RT(val) , BIN_FD(val), BIN_SA(val)/*0 Modificat Nel*/));
